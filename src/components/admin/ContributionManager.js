...
const ContributionManager = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post('https://roking-server.onrender.com/api/payments', { payments: validPayments }, {
        headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
      });
  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      ...
      const response = await axios.post('https://roking-server.onrender.com/api/payments/upload', formData, {
        headers: {
          'Content-Type': 'multipart/form-data',
          Authorization: `Bearer ${localStorage.getItem('token')}`
        }
