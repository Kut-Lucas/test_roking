...
const ApproveMembers = () => {
  ...
  useEffect(() => {
    ...
    axios.get('https://41.57.106.76:8445/api/pending-members', { withCredentials: true })
      .then(response => {
  ...
  const handleApprove = useCallback((id) => {
    axios.post(`https://41.57.106.76:8445/api/approve-member/${id}`, {}, { withCredentials: true })
      .then(() => {
  ...
  const handleReject = useCallback((id) => {
    axios.post(`https://41.57.106.76:8445/api/reject-member/${id}`, {}, { withCredentials: true })
      .then(() => {
        setPendingMembers(pendingMembers.filter(member => member.user_id !== id));
      })
