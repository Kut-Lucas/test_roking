...
const Minutes = () => {
  ...
  useEffect(() => {
    const fetchMinutes = async () => {
      try {
        const response = await axios.get('https://roking-server.onrender.com/api/getMinutes');
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        accessor: 'file_url',
        Cell: ({ value }) => (
          <a href={`https://roking-server.onrender.com/uploads/${value}`} target="_blank" rel="noopener noreferrer">
            View PDF
          </a>
        ),
      },
